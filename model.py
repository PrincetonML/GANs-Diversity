...
class DCGAN(object):
  def __init__(self, sess, input_height=108, input_width=108, crop=True,
         ...
         input_fname_pattern='*.jpg', checkpoint_dir=None, sample_dir=None, train=False):
    """
  ...
  def model_dir(self):
    return "{}_{}_{}_{}_{}".format(
        ...
        self.output_height, self.output_width, self.df_dim)
  ...
  def get_collisions(self, config, nbatch, topK):
    """ Compute the inception score. sample_num should be quite large but smaller than the number of training images """
      ...
      for i in range(n_image):
        for j in range(i+1, n_image):
          ...
          if len(queue) == 0 or -1*dist > queue[0][0]:
            hq.heappush(queue, (-1*copy.deepcopy(dist), copy.deepcopy(images[i]), copy.deepcopy(images[j])))
            if len(queue) > topK:
              hq.heappop(queue)
